...
- Blank first-time setup for your name, gender, training location, experience level, primary fitness goal, weekly workout target, chosen rest weekdays, and optional starting weight. Phones use four short steps with Back navigation and a final review; nothing is saved until you build your plan.
...
- Build a recurring weekly split in **My workout plan → Edit weekly plan**: assign muscle groups to Monday–Sunday, choose exercises, adjust sets and reps, and schedule recovery days. The saved exercises and rest days repeat in all upcoming weeks until you change them. Each experience level keeps its own routine. Switching Gym/Home keeps the schedule and muscle focus, substitutes suitable equipment alternatives, and restores the original exercises when you switch back. Suggested home strength workouts use gym equivalents in Gym mode; deliberately chosen bodyweight movements and shared core or recovery exercises can remain the same.
...
- Choose **Alternatives** beside any daily exercise when equipment is busy or you want another movement. Filter by equipment, compare muscle focus, and preview the replacement’s targets and illustration. Save for that date only or the same weekday every week; unrelated exercise choices stay saved. Alternatives also work inside an active workout, where the timer pauses while choosing and the swap applies only to that session. Uncheck an exercise’s completed sets before replacing it.
...
- Change goals from the dashboard or **Settings → Goals & suggested plan** and review the exact week that will be saved. Changing the training days defaults to adjusting your saved routine: workouts retain their order and targets, fewer days keep the first workouts, and extra days use suggestions. You can instead choose a fresh suggested plan or explicitly keep the current schedule. Adjusting or replacing applies your selected rest days and replaces date edits from this week onward; keeping saves preferences while preserving the existing schedule. Every day remains editable.
...
Open **My workout plan → Review sheet plan → Use this 6-day plan** (on phones, **Plan → Manage your plan → 6-day sheet plan**) to review and apply the supplied **6-Day YouTube Gym Playlist**. You can inspect each exercise illustration, edit its targets, and choose Saturday's cardio before applying. Selecting **At the gym**, **6 days**, and **Build muscle & lose fat** also uses this sheet for the suggested plan; saved custom routines remain until you choose to replace them.
