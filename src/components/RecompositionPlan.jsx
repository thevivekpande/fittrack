...
export default function RecompositionPlan({ level, gender, restDays, hasCustomPlan, onApply, onCancel }) {
  ...
  return <div className="recomposition-plan">
    ...
    <label className="recomposition-cardio">Weekly rest day<TouchSelect aria-label="Weekly rest day" disabled={saving} value={plans.findIndex(day => day.rest)} onChange={event => { setPlans(previous => applyRestDaySchedule(previous, [Number(event.target.value)])); setDemoId(null); }}>{DAYS.map((day,index) => <option key={day} value={index}>{day}</option>)}</TouchSelect></label>
    ...
    {plan.rest ? <div className="recomposition-rest"><Leaf size={32}/><h4>Leave room to recover.</h4><p>No workout is scheduled on {DAYS[dayIndex]}. Rest does not create a completed session or add activity to your progress.</p></div>
      : demo ? <div className="recomposition-demo"><button type="button" className="text-button" onClick={() => setDemoId(null)}><ArrowLeft size={15}/>Back to {DAYS[dayIndex]}</button><h4>{demo.name}</h4><TargetSummary plan={plan} exercise={demo} showRest/><Suspense fallback={<div className="demo-loading">Preparing your movement illustration…</div>}><ExerciseDemo exerciseId={demo.id} group={demo.group} gender={gender} movement={demo.movement} name={demo.name} equipment={demo.equipment}/></Suspense><ol className="recomposition-cues">{demo.instructions.map(instruction => <li key={instruction}>{instruction}</li>)}</ol><ExerciseVideoLinks exercise={demo} links={plan.exerciseVideoLinks?.[demo.id]}/></div>
        : <>
          {plan.exerciseIds.some(id => ['treadmill-walk', 'stationary-bike'].includes(id)) && <label className="recomposition-cardio">Your cardio finish<TouchSelect aria-label="Cardio choice" value={plan.exerciseIds.includes('stationary-bike') ? 'stationary-bike' : 'treadmill-walk'} onChange={event => changeCardio(event.target.value)}><option value="treadmill-walk">Treadmill walking</option><option value="stationary-bike">Stationary cycling</option></TouchSelect></label>}
