...
export default function PhoneLibrary({ exercises, trainingPlace, query, onQuery, filter, onFilter, todayIds, onDemo, onAdd }) {
  ...
  return <div className="phone-library">
    ...
    <div className="phone-library-tools"><div className="phone-search"><Search size={20}/><input type="search" aria-label="Search exercises" placeholder="Search exercises or muscles" value={query} onChange={event=>onQuery(event.target.value)}/>{query&&<button aria-label="Clear search" onClick={()=>onQuery('')}><X size={18}/></button>}</div><div className="phone-library-filter"><label>Muscle group<select aria-label="Filter by muscle group" value={filter} onChange={event=>onFilter(event.target.value)}>{['All exercises',...new Set(exercises.map(exercise=>exercise.group))].map(group=><option value={group} key={group}>{muscleLabel(group)}</option>)}</select></label><span role="status">{results.length} {results.length===1?'exercise':'exercises'}</span></div></div>
