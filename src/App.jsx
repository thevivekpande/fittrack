...
import { Activity, AlertCircle, ArrowRight, ArrowUpRight, Award, Bell, CalendarDays, Check, CheckCheck, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, Clock3, Dumbbell, Flame, Footprints, HardDrive, Heart, Home, LayoutDashboard, Leaf, Menu, Pause, Play, Plus, Search, SlidersHorizontal, Settings, Sparkles, Target, Timer, TrendingUp, Trophy, UserRound, X, Zap } from 'lucide-react';
...
function Workspace({data,setData,returning,lastVisit,saving,saveError,saveConflict,retry}) {
  ...
  const suggestedPlans = getSuggestedWeekPlan({ fitnessGoal: profile.fitnessGoal, gender:profile.gender, level, trainingPlace, weeklyGoal: profile.goal });
  const basePlans = weeklyPlans[weeklyScope] || suggestedPlans;
  const plans = resolveWeekPlans({level,trainingPlace,fitnessGoal:profile.fitnessGoal,gender:profile.gender,weeklyGoal:profile.goal,weeklyPlans,customPlans,dates});
  const todayPlans = resolveWeekPlans({level,trainingPlace,fitnessGoal:profile.fitnessGoal,gender:profile.gender,weeklyGoal:profile.goal,weeklyPlans,customPlans});
  ...
  const useRecompositionPlan = plans => {
    setData(previous => ({
      ...
      trainingPlace: 'gym',
      profile: { ...previous.profile, fitnessGoal: 'body-recomposition', goal: 6 },
    }));
    setFilter('All exercises'); setQuery(''); setWeekOffset(0); setSelectedDay(0);
    setModal(null); navigate('plan');
    notify('Your six-day recomposition plan is saved. Sunday stays a rest day.');
  };
  const saveTrainingGoal = ({ fitnessGoal, gender, weeklyGoal, applySuggestion }) => {
    setData(previous => updateTrainingGoal(previous, { fitnessGoal, gender, weeklyGoal, applySuggestion, trainingPlace, level }));
    setWeekOffset(0);
    setModal(null);
  ...
  return <div className="app-shell">
    ...
    {modal&&<Modal title={modal.type==='demo'?modal.exercise.name:modal.type==='workout'?'Your workout':modal.type==='settings'?'Make this space yours.':modal.type==='help'?'A little help getting started.':modal.type==='customize'?'Make this workout yours.':modal.type==='weekly'?'Build your weekly split.':modal.type==='goals'?'What are you training for?':modal.type==='recomposition'?'Your 6-day recomposition planner':modal.type==='reset-weekly'?'Restore your suggested program?':modal.type==='reload'?'Load your latest saved progress?':'You showed up. You got stronger.'} onClose={()=>setModal(null)} wide={modal.type==='workout'} weekly={modal.type==='weekly'} goal={modal.type==='goals'||modal.type==='recomposition'}>
      {modal.type==='demo'&&<><div className="demo-modal-meta"><span className="pill">{muscleLabel(modal.exercise.group)}</span><span>{modal.exercise.equipment}</span><span><Clock3 size={14}/>{modal.exercise.duration} min</span></div><Suspense fallback={<div className="demo-loading"><Activity className="loading-pulse" size={28}/>Setting up your movement preview…</div>}><ExerciseDemo gender={profile.gender} movement={modal.exercise.movement} name={modal.exercise.name} equipment={modal.exercise.equipment}/></Suspense><h3 className="form-heading">Make every rep count</h3><ol className="instruction-list">{modal.exercise.instructions.map((text,i)=><li key={i}><span>{i+1}</span>{text}</li>)}</ol><ExerciseVideoLinks exercise={modal.exercise} links={modal.videoLinks}/><div className="demo-footer"><span><Sparkles size={15}/> Make room for this in your next session.</span><button className="button button-green" disabled={todayPlan.exerciseIds.includes(modal.exercise.id)} onClick={()=>addToToday(modal.exercise)}>{todayPlan.exerciseIds.includes(modal.exercise.id)?<><Check size={15}/>In today’s plan</>:<><Plus size={15}/>Add to today</>}</button></div></>}
      ...
      {modal.type==='recomposition'&&<RecompositionPlan level={level} gender={profile.gender} hasCustomPlan={Boolean(weeklyPlans[`gym:${level}`])||Object.keys(customPlans).some(key=>key.startsWith(`gym:${level}:`)&&key.slice(`gym:${level}:`.length)>=weekStart)} onApply={useRecompositionPlan} onCancel={()=>setModal(null)}/>}
...
function ExerciseRow({exercise,index,plan,onDemo}) {
  ...
  return <div className="exercise-row"><span className="exercise-number">{String(index+1).padStart(2,'0')}</span><button className="exercise-thumbnail" onClick={()=>onDemo(exercise)} aria-label={`Preview ${exercise.name}`}><img src={exercise.image} alt="" loading="lazy"/><span><Play size={12} fill="currentColor"/></span></button><div className="exercise-row-name"><button onClick={()=>onDemo(exercise)}>{exercise.name}</button><span>{muscleLabel(exercise.group)}<i/> <TargetSummary plan={plan} exercise={exercise}/></span></div><button className="demo-link" onClick={()=>onDemo(exercise)}><span className="demo-link-icon"><Play size={11} fill="currentColor"/></span>View demo<ChevronRight size={14}/></button></div>;
}
...
function WorkoutSession({session,setSession,onComplete,onDiscard,gender}) {
  ...
  const finishedSets=plan.exerciseIds.reduce((sum,id)=>sum+(completed[id]||[]).filter(index=>index<getExerciseTarget(plan,getExercise(id)).sets).length,0);
