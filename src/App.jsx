...
function Workspace({data,setData,returning,lastVisit,saving,saveError,saveConflict,retry}) {
  ...
  const suggestedPlans = getSuggestedWeekPlan({ fitnessGoal: profile.fitnessGoal, gender:profile.gender, level, trainingPlace, weeklyGoal: profile.goal, restDays:profile.restDays });
  const basePlans = weeklyPlans[weeklyScope] || suggestedPlans;
  const plans = resolveWeekPlans({level,trainingPlace,fitnessGoal:profile.fitnessGoal,gender:profile.gender,weeklyGoal:profile.goal,restDays:profile.restDays,weeklyPlans,customPlans,dates});
  const todayPlans = resolveWeekPlans({level,trainingPlace,fitnessGoal:profile.fitnessGoal,gender:profile.gender,weeklyGoal:profile.goal,restDays:profile.restDays,weeklyPlans,customPlans});
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main id="main-content">
        ...
        {view==='plan'&&<>
          ...
          <div className="plan-banner"><div className="plan-banner-icon"><CalendarDays size={26}/></div><div><h2>{weeklyPlans[weeklyScope]?'Your saved weekly routine':activeGoal?'Your suggested weekly plan':`${activeLevel.label} ${trainingPlace==='home'?'home':'gym'} program`}</h2><p>{weeklyPlans[weeklyScope]?`${basePlans.filter(plan=>!plan.rest).length} training days · Repeats every week · ${activeLevel.label}`:activeGoal?`${activeGoal.label} · ${profile.goal} days a week · ${activeLevel.label}`:trainingPlace==='home'?'Legs on Monday? Chest + core on Tuesday? Make it yours.':'Biceps + shoulders on Monday? Chest + triceps on Tuesday? Make it yours.'}</p></div><button className="button button-green weekly-edit-button" onClick={()=>setModal({type:'weekly'})}><SlidersHorizontal size={15}/>{weeklyPlans[weeklyScope]?'Choose exercises & rest days':'Create my own plan'}</button></div>
          <div className="weekly-plan-status"><span><CalendarDays size={13}/>{weeklyPlans[weeklyScope]?'Your exercises and rest days repeat every week until you change them.':'Choose your own exercises for each weekday, then save once for every upcoming week.'}</span>{weeklyPlans[weeklyScope]&&<button className="text-button" onClick={()=>setModal({type:'reset-weekly'})}>Use suggested program <ArrowRight size={13}/></button>}</div>
    ...
    {modal&&<Modal title={modal.type==='demo'?modal.exercise.name:modal.type==='workout'?'Your workout':modal.type==='settings'?'Make this space yours.':modal.type==='help'?'A little help getting started.':modal.type==='customize'?'Make this workout yours.':modal.type==='weekly'?'Build your weekly split.':modal.type==='goals'?'What are you training for?':modal.type==='recomposition'?'Your 6-day recomposition planner':modal.type==='reset-weekly'?'Restore your suggested program?':modal.type==='reload'?'Load your latest saved progress?':'You showed up. You got stronger.'} onClose={()=>setModal(null)} wide={modal.type==='workout'} weekly={modal.type==='weekly'} goal={modal.type==='goals'||modal.type==='recomposition'}>
      ...
      {modal.type==='weekly'&&<WeeklyPlanEditor initialDay={selectedDay} plans={basePlans} exercises={availableExercises} trainingPlace={trainingPlace} level={level} onSave={saveWeeklyPlan} onCancel={()=>setModal(null)}/> }
