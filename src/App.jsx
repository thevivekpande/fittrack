...
import { Activity, AlertCircle, ArrowLeftRight, ArrowRight, ArrowUpRight, Award, Bell, CalendarDays, Check, CheckCheck, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, Clock3, Dumbbell, Flame, Footprints, HardDrive, Heart, Home, LayoutDashboard, Leaf, Menu, Pause, Play, Plus, Search, SlidersHorizontal, Settings, Sparkles, Target, Timer, TrendingUp, Trophy, UserRound, X, Zap } from 'lucide-react';
...
import { applyExerciseReplacement, replaceSessionExercise } from './exerciseReplacement';
...
function WorkoutSession({session,setSession,onComplete,onDiscard,gender,mobile}) {
  ...
  useEffect(()=>{if(paused||alternativeExercise)return;const timer=setInterval(()=>{setSession(previous=>previous?{...previous,elapsed:previous.elapsed+1}:previous);setRest(previous=>Math.max(0,previous-1));},1000);return()=>clearInterval(timer);},[paused,setSession,alternativeExercise]);
